...
- ~~**`/auth/forgot-password`**: Missing.~~ *(Fixed in Commit #3)*
- ~~**`/auth/reset-password`**: Missing.~~ *(Fixed in Commit #3)*
- `POST /auth/verify-email` (Missing entirely)

### Fake / Mock Implementations (Production Blockers)
- ~~**Frontend Mocks**: The UI flows for Register (`register/page.tsx`), Forgot Password (`forgot-password/page.tsx`), and Reset Password (`reset-password/page.tsx`) are purely mocked using `setTimeout`. They do not connect to any actual backend API.~~ *(Fixed in Commit #4)*
