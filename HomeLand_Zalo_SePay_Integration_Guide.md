...
## 2.3. Room occupancy selector

Trước khi thêm khách vào phòng, UI phải chốt một trong hai chế độ:

- `Nguyên căn`
- `Phòng ghép`

Chế độ này là input cho toàn bộ nhánh sau đó:

- tạo hợp đồng
- gắn `roommates` hoặc `sharedTenants`
- bind `chat_id` Zalo cho đúng khách
- tạo QR SePay / phiếu thanh toán
- sinh thông báo vận hành theo đúng phòng và đúng khách
...
==========
Tôi đã tạo file Markdown đầy đủ cho kiến trúc **HomeLand × Zalo Bot × SePay**, gồm hơn **4.000 dòng** tài liệu kỹ thuật và code mẫu.

:chatgpt-content-reference{index="6"}

Tài liệu bao gồm toàn bộ mapping và code graph từ **Zalo Customer → Zalo Webhook → tự lấy ChatID → Contract/Tenant → Billing → QR SePay → SePay Webhook → Auto Reconciliation → xác nhận Customer + Admin Group**. Phần Zalo được bám theo các tài liệu `getMe`, `getUpdates`, `setWebhook`, `sendMessage` bạn đã cung cấp; `sendMessage` dùng `chat_id` làm đích gửi tin.  Việc `getUpdates` không chạy khi webhook đang được cấu hình cũng đã được tính vào kiến trúc. 

Trong file tôi đã thiết kế riêng phần **Admin → Settings → Integrations** với các nút:
...
Phần SePay được cập nhật theo tài liệu Developer hiện hành. SePay xác nhận webhook transaction được POST realtime; production có thể dùng HMAC-SHA256 với `X-SePay-Signature`, `X-SePay-Timestamp` và ký trên `{timestamp}.{raw_body}`.  SePay cũng hỗ trợ QR VietQR điền trước tài khoản, số tiền và nội dung thanh toán. 

Một quyết định kiến trúc quan trọng trong file là **không dùng số điện thoại làm note thanh toán chính**. Thay vào đó:
...
Nếu A thanh toán thì chỉ nghĩa vụ của A thành `PAID`; phòng vẫn `PARTIAL` cho tới khi B thanh toán. Nếu khách xóa note, hệ thống chuyển transaction thành `NEEDS_REVIEW`, **không tự đánh dấu PAID chỉ vì số tiền giống nhau**.

Tài liệu cũng đã có schema SQL cho:
...
và code Node.js cho Zalo client, webhook normalization, `DK SĐT PHÒNG`, tự bind `chat_id`, notification service, tạo payment code, QR adapter, SePay HMAC verification, chống duplicate transaction, partial payment, overpayment, unmatched transaction, scheduler nhắc thanh toán/hợp đồng và Admin API.

Một điểm tôi cố tình **không giả định**: tài liệu Zalo bạn cung cấp chưa đủ để xác minh chính xác schema endpoint gửi **ảnh QR**. Vì vậy kiến trúc đã tách adapter: bản cơ sở gửi QR URL qua `sendMessage`; khi xác minh được `sendPhoto`/media API chính thức thì chỉ thay adapter, không phải sửa Billing/SePay. `setWebhook` cũng được thiết kế đúng mô hình URL HTTPS public và secret theo tài liệu Zalo. 

Bước triển khai nên theo thứ tự trong tài liệu: **Phase 1 Zalo foundation → Phase 2 Customer Registration/ChatID → Phase 3 Billing/ở ghép → Phase 4 SePay/QR/reconciliation → Phase 5 Scheduler → Phase 6 Admin Integration UI**. Cách chia này giúp test từng boundary trước khi cho hệ thống tự động thu tiền và gửi thông báo production.
==================
