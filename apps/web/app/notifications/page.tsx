  ...
  return (
    <AppShell>
      <div
        ...
        className="flex h-[calc(100vh-4rem)] max-w-[1600px] mx-auto bg-background text-text"
      >
        <div
          data-testid="notifications-sidebar"
          className="w-64 border-r border-border p-4 space-y-2 bg-surface"
        >
          ...
          <Button
            ...
            className="w-full justify-start gap-3"
          >
            <Bell className="w-4 h-4" /> Chưa đọc
          </Button>
        ...
        <div
          ...
          className="flex-1 flex flex-col border-r border-border bg-card"
        >
            ...
            {filteredNotifications.map((n: any) => (
              <div
                ...
                className={`p-4 border-b border-border cursor-pointer hover:bg-surface transition-colors ${
                  selectedNotification?.id === n.id ? "bg-surface" : ""
                } ${n.status !== "READ" ? "border-l-4 border-l-primary bg-primary/5" : ""}`}
              >
                <div className="flex justify-between items-start mb-1">
                  <h4 className={`text-sm ${n.status !== "READ" ? "font-black text-text" : "font-bold text-muted"}`}>
                    ...
                    {n.status !== "READ" && (
                      <span
                        data-testid="notification-read-badge"
            ...
            {filteredNotifications.length === 0 && (
              <div data-testid="notifications-empty-state">
                <EmptyState
                  ...
                  message="Bạn đã đọc hết tất cả thông báo."
                  icon={<Mail size={48} />}
                />
              </div>
            )}
