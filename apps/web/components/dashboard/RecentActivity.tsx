      ...
      <div className="flex flex-col gap-[20px] flex-1">
        {activities.map((act, i) => (
          <div key={i} className="flex items-start gap-[12px]">
            <div className="text-[11px] font-bold leading-[1.35] text-muted/70 w-[82px] shrink-0 mt-[3px] whitespace-pre-line tabular-nums">
              {act.time}
            </div>
            ...
            {act.avatar ? (
              <img
                src={act.avatar}
                alt="Avatar"
                className="w-[32px] h-[32px] rounded-full object-cover shrink-0 border border-border/50"
              />
            ) : (
              act.icon
            )}
            ...
            <div className="flex-1 min-w-0">
              <div className="font-bold text-[13px] text-text leading-snug mb-1 line-clamp-2">
                {act.title}
              </div>
              <div className="text-[12px] text-muted font-medium leading-snug">
                {act.desc}
              </div>
            </div>
            ...
            {act.amount && (
              <div
                className={`font-black text-[12px] shrink-0 ${act.amountColor} mt-[6px]`}
              >
                {act.amount}
              </div>
            )}
