...
const getStatusMeta = (status: string) => {
  switch (status) {
    ...
    case "PENDING":
      return { label: "Chờ thu cọc", tone: "bg-sky-500/10 text-sky-600 border-sky-500/20", dot: "bg-sky-500" };
    case "PAID":
      return { label: "Đã thu cọc", tone: "bg-emerald-500/10 text-emerald-600 border-emerald-500/20", dot: "bg-emerald-500" };
          ...
          <table className="w-full min-w-[960px] text-left border-collapse">
            ...
            <tbody className="divide-y divide-border/40">
              ...
              {!isLoading && !isError && deposits.map((deposit: UI_Deposit) => {
                const statusInfo = getStatusMeta(deposit.status);
