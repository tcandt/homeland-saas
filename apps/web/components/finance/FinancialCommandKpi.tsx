...
import { TrendingUp, TrendingDown, DollarSign, Wallet, Minus, Building, CheckCircle, PieChart, Loader2 } from "lucide-react";
import { useDashboardQuery } from "@/lib/queries/dashboard.queries";
import { useCashFlowQuery, useProfitLossQuery } from "@/lib/queries/finance.queries";

export default function FinancialCommandKpi() {
  const { data: dashboard, isLoading: isLoadingDash } = useDashboardQuery();
  const { data: cashflow, isLoading: isLoadingCf } = useCashFlowQuery();
  const { data: pl, isLoading: isLoadingPl } = useProfitLossQuery();

  if (isLoadingDash || isLoadingCf || isLoadingPl) {
    return (
  ...
  const kpisData = (dashboard as any)?.data?.kpis || {};
        ...
        >
          ...
          <div className="flex items-end justify-between z-10 mt-[8px]">
            <div className="flex items-baseline gap-[4px]">
              <span className="font-black text-[16px] md:text-[18px] text-text leading-none">{kpi.value}</span>
