      ...
      <div className="flex flex-col gap-4 border-b border-border p-[16px] md:flex-row md:items-start md:justify-between md:p-[20px]">
        <div>
          <div className="flex items-center gap-2 text-[11px] font-black uppercase tracking-[0.14em] text-[#8b5cf6]">
            <SearchCheck size={14} /> SePay reconciliation
          </div>
          ...
          <p className="mt-1 max-w-[760px] text-[12px] leading-5 text-muted md:text-[13px]">
            Đọc webhook raw payload, phân loại giao dịch khớp hóa đơn/cọc, thiếu tiền, thừa tiền, sai bank hoặc chưa match payment code.
          </p>
        </div>
        <div className="grid grid-cols-1 gap-2 sm:grid-cols-3 sm:min-w-[470px]">
          ...
          <table className="w-full min-w-[1120px] text-left text-sm">
            <thead className="bg-surface text-[11px] uppercase text-muted">
              <tr>
                ...
                <th className="px-4 py-3 font-black text-right">Tiền vào</th>
                <th className="px-4 py-3 font-black text-right">Phải thu</th>
              ...
              {rows.map((row: any) => (
                <tr key={row.id} className="border-t border-border">
                  ...
                  <td className="px-4 py-3">
                    <span className={`inline-flex items-center gap-1 rounded-full px-2 py-1 text-[11px] font-black ${statusClass[row.status] || "bg-slate-100 text-slate-600"}`}>
...
function Metric({ label, value, tone }: { label: string; value: React.ReactNode; tone?: "success" | "warning" | "danger" | "info" }) {
  const valueClass =
    tone === "success" ? "text-[#059669]" :
    tone === "warning" ? "text-[#f97316]" :
