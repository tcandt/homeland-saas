...
export const depositAdapter = {
  toUI(apiDeposit: any): UI_Deposit {
    return {
      ...
      customerName: apiDeposit.customer?.fullName || 'Khách vãng lai',
      customerPhone: apiDeposit.customer?.phone || '-',
      roomId: apiDeposit.roomId || apiDeposit.room?.id,
      roomCode: apiDeposit.room?.code || apiDeposit.room?.name || 'Chưa gắn phòng',
      buildingId: apiDeposit.room?.buildingId || apiDeposit.room?.building?.id,
      buildingName: apiDeposit.room?.building?.name || '-',
