...
```bash
docker compose --env-file .env.public-testing -f docker-compose.public-testing.yml up -d --build
```

If the images were loaded from tar and you do not want to rebuild:

```bash
docker compose --env-file .env.public-testing -f docker-compose.public-testing.yml up -d
```

## Stop

```bash
docker compose --env-file .env.public-testing -f docker-compose.public-testing.yml down
```

## Notes

- Database password is changed in `.env.public-testing`.
