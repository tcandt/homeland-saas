...
4. Nếu có migration mới, chạy `prisma migrate deploy` đúng một lần từ artifact đã duyệt. Không dùng `db push`, `--force-reset` hoặc seed production trong deploy thường lệ.
